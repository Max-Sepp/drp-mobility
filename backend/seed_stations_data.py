...
def translate_area_with_platforms(
    area_name: str, station_uid: str, platforms_by_station: dict
) -> str:
...
def resolve_areas(
    ...
    point_by_id: dict,
    station_uid: str = "",
    platforms_by_station: dict | None = None,
) -> str | None:
...
def platforms_are_same_level_connected(
    station_uid: str, points_by_station: dict, same_level_paths: dict
) -> bool:
    ...
    plat_point_ids = {
        uid
        for uid, area in pts.items()
        if any(area.startswith(pfx) and area[len(pfx) :] in _DIR_SUFFIX for pfx in _LINE_PREFIX)
    }
...
def build_step_free_graph(
    same_level_rows: list, ramp_rows: list, lifts_rows: list
) -> dict[str, set]:
...
def derive_step_free_access(
    platform_id: str, reachable: set[str], level_access: bool, boarding_ramp: bool
) -> str:
...
def synthesise_escalator_units(
    station_uid: str, escalator_count: int, platforms_by_station: dict
) -> list[dict]:
        ...
        if "platforms_patch" in override:
            ...
            for plat_id, patch in override["platforms_patch"].items():
                if plat_id in plat_by_id:
                    ...
                    print(
                        f"WARNING: station_overrides.json: unknown platform"
                        f" {plat_id!r} on station {name!r}"
                    )
...
def main() -> None:
    ...
    for station in all_stations:
        ...
        if not lift_units and station.get("lifts", 0) > 0:
            lift_units = synthesise_lift_units(
                uid, station["lifts"], platforms_by_station, points_by_station, same_level_paths
            )
        if lift_units:
        ...
        if escalator_count > 0:
            escalator_units = synthesise_escalator_units(uid, escalator_count, platforms_by_station)
